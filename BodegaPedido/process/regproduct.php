<html>
    ...
    <body>
        ...
        $platilloProd= $_POST['prod-platillo'];
        $priceProd= $_POST['prod-price'];
        $descripcionProd= $_POST['prod-descripcion'];
        ...
        if(!$nameProd=="" && !$platilloProd=="" && !$priceProd=="" && !$descripcionProd=="" && !$codePProd=="" && !$cateProd=="" && !$stockProd=="" && !$_FILES['img']['name']==""){
            $verificar=  ejecutarSQL::consultar("select * from platillos where codigoPlatillo='".$codeProd."'");
            $verificaltotal = mysql_num_rows($verificar);
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$_FILES['img']['name'])){
                    if(consultasSQL::InsertSQL("platillos", " platillo, tipoPlatillo, precio, descripcion, imagen, codigoProveedor, CodigoCat, Nombre, Stock", "'$nameProd','$platilloProd','$priceProd', '$descripcionProd','".$_FILES['img']['name']."','$codePProd','$cateProd', '$adminProd','$stockProd'")){
                       echo '
